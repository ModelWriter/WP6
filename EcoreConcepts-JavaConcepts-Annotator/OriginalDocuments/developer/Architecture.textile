...
* It allows the verification of any access or modification using the associated @IPermissionAuthority@. The permission authority is used to lock model elements (both diagram and semantic elements), and to check if you have read and/or write access to each element and feature before access or modification.
